...
local _VERSION = "0.0.0-poc.1.1.3"
...
local function get_videodevice()
    local monitor = peripheral.find("monitor")
    if monitor then
        monitor.setTextScale(settings.get("youcube.monitor_scale") or 1)
        return monitor
    end
    return term.current and term.current() or term
end
...
local videodevice = get_videodevice()

local function with_videodevice(fn)
    local current = term.current and term.current()
    if current and videodevice ~= current then
        current = term.redirect(videodevice)
    else
        current = nil
    end
...
local function reset_videodevice()
    with_videodevice(libs.youcubeapi.reset_term)
end
...
local function play(url)
    ...
    local display_width, display_height = videodevice.getSize()

    if not args.no_video then
        youcubeapi:request_media(url, display_width, display_height)
    else
    ...
    local video_buffer = libs.youcubeapi.Buffer.new(
        libs.youcubeapi.VideoFiller.new(youcubeapi, data.id, display_width, display_height),
